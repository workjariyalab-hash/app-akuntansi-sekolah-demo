...
| **Diturunkan dari** | PRD Sistem Akuntansi Sekolah v1.0 (yang menurunkan v2.4) |
| **Perubahan utama v1.1** | Perluasan modul Forecasting: **Proyeksi Anggaran Tahunan** (forecasting keuangan menyeluruh berbasis General Ledger — proyeksi pendapatan & beban + rekomendasi anggaran tahun depan). Lihat Bagian 3 (DF9–DF14), 4.15, 5.4, dan Riwayat Revisi |
| **Penulis** | [Nama Anda] |
...
| D: Budgeting & Forecasting | **Diperluas (v1.1)**: ditambah kemampuan **Proyeksi Anggaran Tahunan** berbasis General Ledger (DF9–DF14). Forecasting kini mencakup sisi beban dan rekomendasi anggaran tahun depan, tidak hanya sisi penerimaan |
...
### Modul D.F+ : Forecasting Keuangan & Proyeksi Anggaran Tahunan (Perluasan v1.1)

> **Konteks**: Modul Forecasting pada v2.4/v1.0 (DF1–DF8) berfokus pada **sisi penerimaan** — memproyeksikan penagihan (billing forecast) dan penerimaan kas (collection forecast) dari SPP. Perluasan v1.1 menambahkan **forecasting keuangan menyeluruh**: memproyeksikan **pendapatan dan beban** untuk tahun buku berikutnya berbasis data **General Ledger**, lalu menghasilkan **rekomendasi anggaran (budget) tahunan per kategori beban**. Ini menjawab kebutuhan "berapa anggaran yang perlu disusun untuk satu tahun ke depan".

| ID | Kebutuhan | Prioritas |
|---|---|---|
| DF9 | Sistem menghasilkan **Proyeksi Anggaran Tahunan** untuk tahun buku berikutnya, dihitung dari **realisasi tahun berjalan** (saldo akun pendapatan 4xxx dan beban 5xxx di Buku Besar) dikalikan **asumsi pertumbuhan** yang dapat dikonfigurasi | P0 |
| DF10 | Foundation Admin/Akuntan dapat menyetel **asumsi proyeksi**: (a) pertumbuhan jumlah siswa, (b) kenaikan SPP/tarif, (c) inflasi biaya operasional, (d) kenaikan gaji — masing-masing dalam persen | P0 |
| DF11 | Sistem menghitung **proyeksi pendapatan** per akun pendapatan dan **proyeksi beban** per akun beban menggunakan aturan penggerak (driver) per jenis akun (detail D.F+.1) | P0 |
| DF12 | Sistem menampilkan **ringkasan proyeksi**: Total Proyeksi Pendapatan, **Total Anggaran Beban (tahun depan)**, Proyeksi Surplus/Defisit, dan Margin Surplus | P0 |
| DF13 | Sistem menampilkan **perbandingan Tahun Ini vs Tahun Depan** (pendapatan, beban, surplus) dalam bentuk grafik, serta **tabel rekomendasi anggaran beban per kategori** (Realisasi tahun ini → Anggaran tahun depan → Δ%) | P0 |
| DF14 | Foundation Admin/Akuntan dapat **mengekspor rekomendasi anggaran** (Excel) dan **mengirimkannya ke modul Budgeting** sebagai dasar penyusunan rencana anggaran (mengisi `budgets`/`plannedAmount`) | P1 |

#### D.F+.1 Metodologi Proyeksi Anggaran Tahunan

**Dasar perhitungan:** realisasi (atau estimasi) tahun berjalan per akun di General Ledger — pendapatan (kelompok 4xxx) dan beban (kelompok 5xxx).

**Proyeksi Pendapatan per akun:**
```
SPP & Kegiatan Siswa     = realisasi × (1 + pertumbuhanSiswa) × (1 + kenaikanTarif)
Dana BOS                 = realisasi × (1 + pertumbuhanSiswa)        // dana per-siswa dari pemerintah
Pendapatan Lainnya       = realisasi × (1 + kenaikanTarif)
```

**Proyeksi Beban per akun (= rekomendasi anggaran):**
```
Gaji & Tunjangan, BPJS   = realisasi × (1 + kenaikanGaji) × (1 + pertumbuhanSiswa × 0,5)
Penyusutan Aset Tetap    = realisasi                                  // mengikuti jadwal aset (tetap)
Pemeliharaan Gedung      = realisasi × (1 + inflasiBiaya)
Beban Operasional Lain   = realisasi × (1 + inflasiBiaya) × (1 + pertumbuhanSiswa × 0,5)
```

**Hasil:**
```
Total Anggaran Beban Tahun Depan = Σ proyeksi beban per akun
Proyeksi Surplus/Defisit         = Total Proyeksi Pendapatan − Total Anggaran Beban
Margin Surplus (%)               = Proyeksi Surplus / Total Proyeksi Pendapatan × 100
```

**Asumsi default:** pertumbuhan siswa 5%, kenaikan tarif 8%, inflasi biaya 6%, kenaikan gaji 7% — seluruhnya dapat diubah pengguna untuk skenario yang berbeda.

#### D.F+.2 Komponen Tampilan (halaman Forecasting → tab "Proyeksi Anggaran Tahunan")
1. **Panel asumsi**: empat input persentase (pertumbuhan siswa, kenaikan tarif, inflasi biaya, kenaikan gaji) yang langsung memperbarui hasil.
2. **Summary cards**: Proyeksi Pendapatan · Total Anggaran Beban · Proyeksi Surplus/Defisit · Margin Surplus.
3. **Grafik perbandingan**: bar chart Tahun Ini vs Tahun Depan (Pendapatan, Beban, Surplus).
4. **Tabel rekomendasi anggaran**: per akun beban — Realisasi tahun ini · Anggaran tahun depan · Δ% — beserta total.
5. **Aksi**: Ekspor Excel + tombol "Jadikan Budget" (mengirim ke modul Budgeting).

> **Catatan implementasi**: Fitur ini **telah diimplementasikan** pada web app (halaman **Forecasting**, dibagi menjadi dua tab: *Proyeksi Penerimaan* untuk DF1–DF8 dan *Proyeksi Anggaran Tahunan* untuk DF9–DF14). Logika proyeksi berada di modul GL bersama (`lib/gl.ts → projectAnnualBudget`).
...
### 4.15 Perencanaan Anggaran Tahunan (Annual Budget Forecast) — v1.1

1. Foundation Admin/Akuntan membuka menu **Forecasting** → tab **Proyeksi Anggaran Tahunan**.
2. Sistem mengambil realisasi tahun berjalan dari Buku Besar (pendapatan 4xxx & beban 5xxx).
3. Pengguna menyetel asumsi: pertumbuhan siswa, kenaikan tarif, inflasi biaya, kenaikan gaji.
4. Sistem menghitung proyeksi pendapatan & beban tahun depan, lalu menampilkan ringkasan (Pendapatan · **Total Anggaran Beban** · Surplus/Defisit · Margin) dan grafik perbandingan Tahun Ini vs Tahun Depan.
5. Pengguna meninjau **tabel rekomendasi anggaran beban per kategori** dan menyesuaikan asumsi untuk skenario berbeda.
6. Pengguna mengekspor laporan (Excel) atau menekan **"Jadikan Budget"** untuk mengirim rekomendasi ke modul Budgeting sebagai dasar rencana anggaran.
...
### 5.4 Budget Forecast Engine (v1.1)

Modul Forecasting diperluas dengan komponen **Annual Budget Forecast Engine** yang bekerja di atas data General Ledger:
- **GL Run-rate Reader**: membaca saldo realisasi akun pendapatan (4xxx) dan beban (5xxx) tahun berjalan sebagai basis proyeksi.
- **Assumption Engine**: menerapkan asumsi pertumbuhan (pertumbuhan siswa, kenaikan tarif, inflasi biaya, kenaikan gaji) dengan **aturan driver per jenis akun** (lihat D.F+.1).
- **Budget Recommender**: menghasilkan rekomendasi anggaran beban per kategori untuk tahun berikutnya, proyeksi surplus/defisit, dan margin.
- **Budget Bridge**: opsi mengirim rekomendasi anggaran ke modul Budgeting (mengisi `budgets.planned_amount`).

Perhitungan bersifat **stateless/on-demand** (dihitung saat asumsi diubah) sehingga tidak perlu tabel baru; hasil dapat disimpan opsional ke `budgets` saat pengguna menekan "Jadikan Budget". Pada implementasi web app, logika ini berada di modul GL bersama (`lib/gl.ts → projectAnnualBudget`).
...
| Fase | Cakupan |
|---|---|
| **Fase 1 (MVP)** | Seluruh fitur Fase 1 dari v2.4 + Kasir dasar (H1–H3, H6, H8) + Simpanan Siswa (I2–I8) + Hutang Usaha dasar (J1–J4, J6, J8) + Penggajian dasar (K1–K7, K10) + Inventaris dasar (L1–L4, L7) + GL penuh (M1–M6 semua ID P0) + Tutup Tahun (M7-1 s/d M7-5) + Rasio dasar (M8-1, M8-4) + **Proyeksi Anggaran Tahunan (DF9–DF13)** + Laporan Keuangan (F7, F8, F11, F12, F13, F14) |
| **Fase 2** | Seluruh fitur Fase 2 dari v2.4 + Kasir lanjutan (H4, H5, H7, H9) + AP lanjutan (J5, J7) + Payroll lanjutan (K8, K9) + Inventaris lanjutan (L5, L6, L8) + Perbandingan periode GL (M4-4, M5-4, M6-4) + Rasio lanjutan (M8-2, M8-3, M8-5) + **Ekspor & bridge anggaran ke Budgeting (DF14)** + Laporan tambahan (F9, F10, F15) |
...
| **Proyeksi Anggaran Tahunan** | Forecasting keuangan menyeluruh (pendapatan + beban) untuk tahun buku berikutnya berbasis realisasi GL × asumsi pertumbuhan, menghasilkan rekomendasi anggaran beban per kategori (v1.1, DF9–DF14) |
| **Run-rate** | Realisasi/estimasi nilai tahun berjalan yang dijadikan basis proyeksi tahun berikutnya |
| **Driver (penggerak)** | Faktor pertumbuhan yang menggerakkan proyeksi sebuah akun (mis. pertumbuhan siswa, kenaikan tarif, inflasi biaya, kenaikan gaji) |
| **Margin Surplus** | Proyeksi Surplus ÷ Proyeksi Pendapatan × 100% |
...
| **Sistem Akuntansi v1.1** | **28 Jun 2026** | Perluasan modul Forecasting: **Proyeksi Anggaran Tahunan** (DF9–DF14) — forecasting keuangan menyeluruh berbasis General Ledger yang memproyeksikan pendapatan & beban tahun depan dan menghasilkan rekomendasi anggaran per kategori beban. Termasuk: metodologi proyeksi & aturan driver per akun (D.F+.1), komponen UI dua-tab (D.F+.2), alur perencanaan anggaran (4.15), Budget Forecast Engine (5.4), pembaruan fasing & glosarium. Tidak menghapus v1.0; merupakan dokumen terpisah |
